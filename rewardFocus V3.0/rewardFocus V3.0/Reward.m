...
%% 定义实验参数
    ...
    %高奖励和低奖励的金额
    highRewardLevel = Shuffle([1*ones(1,96), 0.01*ones(1,24)]);
    lowRewardLevel = Shuffle([0.01*ones(1,96), 1*ones(1,24)]);
...
%% 开始实验
       ...
       Screen('Flip', window);
        ...
        if ~isRunning
            ...
            if answer(trialIndex) == barType(trialIndex)% answer(trialIndex) ==1表示|，answer(trialIndex)==2表示—，barType(trialIndex)==1表示|，==2表示 --
                ...
                DrawFormattedText(window, [num2str(currentReward)], screenRect(3) / 2 + 10, screenRect(4) / 2 - 45 ); % [num2str(currentReward)]
                DrawFormattedText(window,  [ num2str(totalReward)], screenRect(3) / 2 - 65, screenRect(4) / 2 + 5 );   % [ num2str(totalReward)]                
            else %错误反馈
   ...
   Screen('DrawTexture', window, endTex);
    DrawFormattedText(window, [num2str(totalReward)], screenRect(3) / 2 + 208, screenRect(4) / 2 - 45);  %[num2str(totalReward)]
        %DrawFormattedText(window, ['请不要进行操作，并联系主试，谢谢合作~'], 'center', screenRect(4) / 2 + 30 ); 
    keyisdown = 1;
    while(keyisdown) % first wait until all keys are released
         [keyisdown,secs,keycode] = KbCheck;
         WaitSecs(0.001); % delay to prevent CPU hogging
